...
变量还可以配合`readonly`来表示这个是一个只读变量，意思是只能赋值一次  
在无法赋值常量时，但你又想只能设置一次，可以使用`readonly`来实现  
一般用法
```C#
...
数组在定义时，需要决定`类型`与`长度`  
数组里面的内容称为`元素`，数组的第几个就代表第几个元素  
数组一旦定义后，类型与长度均不可以改变，里面的元素会给`类型的默认值`  
关于类型的默认值，可以参考[C# 类型的默认值（C# 参考）](https://learn.microsoft.com/zh-cn/dotnet/csharp/language-reference/builtin-types/default-values)  
